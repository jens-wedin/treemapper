 ...
 */
function createToggle(key: string, legacyKeys: string[], fallback: boolean) {
  const listeners = new Set<() => void>();
  let value = read();

  function read(): boolean {
    const stored = readPreference(key, ...legacyKeys);
...
export const useFlagPreference =
  createToggle('treemapper-show-flags', ['wedin-tree-show-flags', 'wedin-tree-visa-flaggor'], true);
...
export const useIssueMarkPreference =
  createToggle('treemapper-show-issues', ['wedin-tree-show-issues', 'wedin-tree-visa-konsekvenser'], false);
...
export const useAddRelativePreference =
  createToggle('treemapper-add-relative', ['wedin-tree-add-relative', 'wedin-tree-lagg-till-slakting'], false);
