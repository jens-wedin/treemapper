...
beforeAll(() => {
  ...
  api = createTreeApi(fixedTree(createDb(dbPath)));
});
...
describe('GET /api/tree/:id', () => {
  ...
  it('validates and 404s', async () => {
    expect((await api.request('/api/tree/I3?up=9')).status).toBe(400);   // ancestors: 8 at most
    expect((await api.request('/api/tree/I3?down=6')).status).toBe(400); // descendants: 5 at most
